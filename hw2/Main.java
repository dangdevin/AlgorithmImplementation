...
public class Main {
	...
	// methods to get full credit.
		...
		int sizeOfMap = map.getSize();		//size of the map
		int largestLinkedListSize = 0;

		for(int i = 0; i < sizeOfMap; i++)
		{
			if(map.countCollisions(i) > largestLinkedListSize)
			{
				largestLinkedListSize = map.countCollisions(i);
			}
		}

		return largestLinkedListSize;
	}
